...
class Transaction(models.Model):
    ...
    def send_user_email(self, subject, template_name, extra_context=None):
        try:
            ...
            logger.error(f"[EMAIL ERROR] Failed to send user email: {e}")

    # -----------------------------------------------------
    #                ADMIN EMAIL (NOW SAFE)
    # -----------------------------------------------------
    def send_admin_email(self, subject, template_name):
        try:
            ...
            msg = EmailMultiAlternatives(
                ...
                body="",
                from_email=settings.DEFAULT_FROM_EMAIL,
                to=[settings.ADMIN_EMAIL],   # Ensure exists
            )
            msg.attach_alternative(html_content, "text/html")
            msg.send(fail_silently=False)

        except Exception as e:
            logger.error(f"[EMAIL ERROR] Failed to send admin email: {e}")

    # -----------------------------------------------------
    #              SAVE OVERRIDE (EMAIL SAFE)
    # -----------------------------------------------------
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            # Only trigger emails if status has changed
            if old_status != self.status:
                send_notification = True

        super().save(*args, **kwargs)

        # Send notification emails safely
        if send_notification:
            try:
                ...
                # Final safeguard — prevents save() errors breaking admin panel
                logger.error(f"[EMAIL ERROR] Failed in save() email notifications: {e}")
...
#             body="",
#             from_email=settings.DEFAULT_FROM_EMAIL,
#             to=[settings.ADMIN_EMAIL],   # Make sure this exists in settings.py
#         )
#         msg.attach_alternative(html_content, "text/html")
#         msg.send(fail_silently=False)
    ...
    def approve(self):
        if self.user_profile.balance >= self.amount:
            ...
            try:
                ...
                import logging
                logger = logging.getLogger(__name__)
                logger.error(f"Failed to send withdrawal approval email for user {self.user_profile.user.username}: {str(e)}")
